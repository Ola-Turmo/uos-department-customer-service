 ...
 */
function isHighValueAccount(customer: CustomerProfile): boolean {
  return (
    customer.slaTier === 'enterprise' ||
 ...
 */
function getMaxAutonomousRefund(
 ...
 * VIP/enterprise customers get highValueAccountMultiplier × the base limit.
 */
function getMaxAutonomousCredit(
  policy: AutonomousPolicy,
  customer: CustomerProfile,
): number {
 ...
 */
function isAlwaysRequireHumanCategory(
  category: string,
  policy: AutonomousPolicy,
): boolean {
...
export class PolicyEngine {
  ...
  constructor(policy: AutonomousPolicy = DEFAULT_AUTONOMOUS_POLICY) {
    this.policy = { ...policy };
  }
  ...
  check(
    ...
    if (
      shouldEscalateOnChurnRisk(
        customer.churnRisk,
        this.policy.autoEscalateOnChurnRisk,
      )
    ) {
      return {
        allowed: false,
    ...
    if (
      action.type === 'send_response_draft' ||
...
function shouldEscalateOnChurnRisk(
  churnRisk: CustomerProfile['churnRisk'],
  autoEscalateOnChurnRisk: AutonomousPolicy['autoEscalateOnChurnRisk'],
): boolean {
