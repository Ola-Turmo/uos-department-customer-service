 ...
 */
function calculateMatchScore(ticket: Ticket, node: { title: string; content: string }): number {
  ...
  const ticketWords = new Set(
    ticketText.split(/\s+/).filter(w => w.length > 2)
  );
 ...
 */
export function autoLinkToDocs(
  ticket: Ticket,
  graph: KnowledgeGraphService
): string[] {
  const allNodes = graph.getAllNodes();
  const matchedIds: Array<{ id: string; score: number }> = [];
